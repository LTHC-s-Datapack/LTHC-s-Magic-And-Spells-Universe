#Rune
schedule function lthc.lmasu:lthc.lmasu/rune/run 67s replace

#Elemental Tools Power
schedule function lthc.lmasu:lthc.lmasu/elemental_tools/run_power 4s replace
schedule function lthc.lmasu:lthc.lmasu/elemental_tools/run_power_tick 15t replace

#Crafting
schedule function lthc.lmasu:lthc.lmasu/craft/rune/crafter 1t replace
schedule function lthc.lmasu:lthc.lmasu/craft/magic_books/crafter 2t replace

#Potions
schedule function lthc.lmasu:lthc.lmasu/potions/potion_craft_main 4t replace

#forging
schedule function lthc.lmasu:lthc.lmasu/elemental_tools/forging 3t replace

#"Basics"
schedule function lthc.lmasu:lthc.lmasu/main/basics 10t replace

#Indicator
schedule function lthc.lmasu:lthc.lmasu/main/indicator 5t replace

#Call Message
schedule function lthc.lmasu:lthc.lmasu/init_lthc/message 3s replace

#Fk player init
schedule function lthc.lmasu:lthc.lmasu/init_lthc/fk_player_init 4s replace

#Boss summon controller
schedule function lthc.lmasu:lthc.lmasu/bosses/summon_overworldian 1t replace
schedule function lthc.lmasu:lthc.lmasu/bosses/summon_netherian 1t replace

#Mob levels
schedule function lthc.lmasu:lthc.lmasu/mob_levels/manager 9t replace

#Lmasu Lag Clearer
schedule function lthc.lmasu:lthc.lmasu/lmasu_clear_lag/lmasu_clear_lag 120s replace
